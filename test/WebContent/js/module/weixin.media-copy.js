...
function doUploadImg(){
	$('#img_form').form('submit', {
		...
		success: function(json){
			var data = eval('(' + json + ')');
			if (data.success){
               	var hasTw=$('#tw_edit').is(':visible');//判断当前窗口是单||多图文
               	if(hasTw===true){
					$("#tw_thumb_media_id").val(data.media_id);
					$("#tw_cover").val(data.fileName);
					$("#tw_fm").html("<img src='../upload/"+data.fileName+"' style='width:230px;height:98px;'>");
				}else{
					$("#more_thumb_media_id").val(data.media_id);
					$("#more_cover").val(data.fileName);
					$("#more_fm").html("<img src='../upload/"+data.fileName+"' style='width:230px;height:98px;'>");
				}
			...
			$.ajax({
				...
				success:function(row){
					...
					if(typeof(row.cover)!=='undefined'&&row.cover!==''){
						$("#more_fm").html("<img src='../upload/"+row.cover+"' style='width:230px;height:98px;'>");
					}
					//表单加载
					$('#more_form').form('load',{
...
function doSubmitTW(){
    $('#tw_form').form('submit',{
        ...
        success: function(json){
	        var data = eval('(' + json + ')');
	        if (data.type!==null&&data.type!==''&&typeof(data.type)!='undefined'){
				$('#tw_data').datagrid('reload');
			 	$("#tw_edit").dialog('close');
		 	 	$.messager.show({title:'消息提示',msg:'提交成功！',showType:'show'});
	        }else{
...
function mediaNameView(value,row,index){
   	if(row.type=='image'||row.type=='thumb'){
		if(typeof(row.fileName)!="undefined"){
			value="<img src='../upload/"+row.fileName+"' style='width:200px;height:100px;'/>";
		}
	}
	return value;
}
...
function midiaList(){
   	$('#media_data').datagrid({
        ...
        columns:[[
			...
			{align:'center',field:'name',width:'200',formatter:mediaNameView,title:'素材内容'},
